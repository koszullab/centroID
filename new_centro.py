...
#        self.cis_free_im_init = np.float32(self.cis_free_im_init)
        ...
        for id_cont in ContFrags:
            if not (id_cont == 17):
                ...
                tick_kb = []
                tick = range(0, len(cont_frag))
                end_frags_kb = []
                print "loading fragments ..."
                print "n frags = ",len(cont_frag)
                for id_f in xrange(0, len(cont_frag)):
                    f = cont_frag[id_f]
                    lkb = f.length_kb
...
#                    tick_kb.append(start)
        ...
        for id_cont in xrange(1, len(self.dict_contigs)+1):
            ...
            if not (id_cont == 17):
               ...
               intra = self.cis_free_im_init[np.ix_(coord_cont[id_cont], coord_cont[id_cont])]
        ...
        for id_cont in self.dict_contigs:
            if not (id_cont == 17):
                ...
                if self.is_max_level:
                    mat_tmp = self.cis_free_im_init[np.ix_(coord_cont[id_cont], coord_cont[id_cont])]
                    mat_tmp[:, :] = self.mean_value_trans
                    mat_tmp = np.random.poisson(mat_tmp)
                    self.cis_free_im_init[np.ix_(coord_cont[id_cont], coord_cont[id_cont])] = mat_tmp
                else:
                    self.cis_free_im_init[np.ix_(coord_cont[id_cont], coord_cont[id_cont])] = self.mean_value_trans
...
#        self.im_cis_free_curr = cuda.scn(tmp_trans_free, n_iter)
    ...
    def plot_sub_matrices(self,folder, thresh, thresh_im_init):
        ...
        fig = plt.figure()
        plt.imshow(corr_im, interpolation='nearest',vmin=0,vmax=thresh * corr_im.max())
        fig.savefig(os.path.join(folder,str(self.level) + "_full_im_corr_" +str(thresh)+ '.pdf'))
        plt.close()
        fig = plt.figure()
        plt.imshow(self.im_curr, interpolation='nearest',vmin=0,vmax=thresh_im_init * self.im_curr.max())
        fig.savefig(os.path.join(folder,str(self.level) + "_full_im_init_" +str(thresh_im_init) + '.pdf'))
        plt.close()
        ...
        for id_cont_X in self.dict_contigs:
            ...
            fig = plt.figure()
            plt.imshow(tmp_c,interpolation='nearest',vmin=0,vmax=thresh * tmp_c.max())
            fig.savefig(os.path.join(folder, str(self.level) + "_corr" + name_contig + '.pdf'))
            plt.close()
    ...
    def gpu_update_data_scn(self,cuda_gen,iteration, fact_sub_sampling):
        ...
        self.im_cis_free_curr = cuda_gen.generate_new_matrix(n_iter_scn, do_random,fact_sub_sampling)
...
class analysis():
    def __init__(self, pyramid_hic, output_folder, n_levels, start):
        ...
        for lev in self.level.keys():
            self.level[lev].load_data(self.pyramid)

        self.ground_truth = {1: (151465, 151582),
                             2: (238207, 238323),
                             3: (114385, 114501),
                             4: (449711, 449821),
                             5: (151987, 152104),
    ...
    def plot_4_sup_material(self, size_sub_win_bp, folder, thresh, thresh_im_init,thresh_im_norm):
        iter_scn = 10
        list_level = self.level.keys()
        for lev in list_level:
            ...
            for id_cont_X in level_pyr.dict_contigs:
                intra_coord = level_pyr.inter_coord[id_cont_X][id_cont_X]
                im_norm[intra_coord] = level_pyr.perform_scn(level_pyr.im_init[intra_coord], iter_scn)
            siz_fig = 9
            fig = plt.figure(figsize=(siz_fig,siz_fig))
            plt.hist(level_pyr.distri_frag, 300)
            plt.title('fragment size distribution (bp)')
            fig.savefig(os.path.join(folder,str(level_pyr.level) + "_frag_length_distribution"'.pdf'))
            plt.close()


            fig = plt.figure(figsize=(siz_fig,siz_fig))
            plt.imshow(corr_im, interpolation='nearest',vmin=0,vmax=thresh * corr_im.max())
            fig.savefig(os.path.join(folder,str(level_pyr.level) + "_full_im_corr_" +str(thresh)+ '.pdf'))
            plt.close()

            fig = plt.figure(figsize=(siz_fig,siz_fig))
            plt.imshow(level_pyr.im_curr, interpolation='nearest',vmin=0,
                       vmax=thresh_im_init * level_pyr.im_curr.max() * 3 ** (lev - 3))
            fig.savefig(os.path.join(folder,str(level_pyr.level) + "_full_im_init_" +str(thresh_im_init) + '.pdf'))
            plt.close()
            fig = plt.figure(figsize=(siz_fig,siz_fig))
            plt.imshow(im_norm, interpolation='nearest',vmin=0,vmax=thresh_im_norm * im_norm.max() * 3 ** (lev - 3))
            fig.savefig(os.path.join(folder,str(level_pyr.level) + "_full_im_norm_" +str(thresh_im_norm) + '.pdf'))
            plt.close()
            ...
            for id_cont_X in level_pyr.dict_contigs:
                ...
                fig = plt.figure(figsize=(siz_fig,siz_fig))
                plt.imshow(tmp_c,interpolation='nearest',vmin=0,vmax=thresh * tmp_c.max())
                fig.savefig(os.path.join(folder, str(level_pyr.level) + "_corr" + name_contig + '.pdf'))
                plt.close()

                if lev == list_level[-1]:
                    tick_score_kb = self.level[lev].dict_contigs[id_cont_X]["tick_kb"]
                    score = self.level[list_level[-1]].all_data[id_cont_X]["score"][id_cont_X]
                    score = np.array(score)
                    data_matlab[name_contig + '_tick_score_bp'] = tick_score_kb
                    data_matlab[name_contig + '_score_rough_estimate'] = score
                    fig = plt.figure(figsize=(siz_fig,siz_fig))
                    plt.bar(tick_score_kb, score,linewidth=2,facecolor='r', edgecolor='r')
                    plt.title('score pre detection '+ name_contig)
                    plt.xlabel('genomic position (bp)')
                    fig.savefig(os.path.join(folder,str(level_pyr.level) + '_' + name_contig + "_score_rough_detect" + '.pdf'))
                    plt.close()
                ...
                for id_cont_Y in self.list_id_chrom:
                    if id_cont_X != id_cont_Y:
                        ...
                        pre_pos_pxl_j = self.bp_coord_2_pxl_coord(pre_pos_bp_j, id_cont_Y, lev)
                        cumul_mat_norm = cumul_mat_norm + im_norm[coord_ij][pre_pos_pxl_i - delta: pre_pos_pxl_i + delta + 1,
                                  pre_pos_pxl_j - delta: pre_pos_pxl_j + delta + 1]
                        cumul_mat_raw = cumul_mat_raw + curr_im[coord_ij][pre_pos_pxl_i - delta: pre_pos_pxl_i + delta + 1,
                                                          pre_pos_pxl_j - delta: pre_pos_pxl_j + delta + 1]
                data_matlab[name_contig + 'intra_corr'] = tmp_c
                data_matlab[name_contig + 'intra_raw'] = tmp_i
                fig = plt.figure(figsize=(siz_fig,siz_fig))
                plt.imshow(cumul_mat_raw,interpolation='nearest',vmin=0,vmax=thresh * cumul_mat_raw.max())
                fig.savefig(os.path.join(folder, str(level_pyr.level) + "_cumul_raw" + name_contig + '.pdf'))
                plt.close()
                fig = plt.figure(figsize=(siz_fig,siz_fig))
                plt.plot(tick, cumul_mat_raw.sum(axis=1))
                plt.xlabel('genomic_position (bp)')
                plt.title('raw data inter sum profile '+ name_contig)
                plt.axvspan(ground_truth[0], ground_truth[1], edgecolor='r', facecolor='r', alpha=1)
                plt.legend(['contact enrichment', 'ground truth'])
                fig.savefig(os.path.join(folder, str(level_pyr.level) + "_sum_profile_cumul_raw" + name_contig + '.pdf'))
                plt.close()
                fig = plt.figure(figsize=(siz_fig,siz_fig))
                ...
                plt.title('raw data inter sum profile '+ name_contig)
                plt.legend(['contact enrichment', 'ground truth'])
                fig.savefig(os.path.join(folder, str(level_pyr.level) + "_max_profile_cumul_raw" + name_contig + '.pdf'))
                plt.close()
                data_matlab[name_contig+'cum_raw'] = cumul_mat_raw
                data_matlab[name_contig+'cum_norm'] = cumul_mat_norm
                fig = plt.figure(figsize=(siz_fig,siz_fig))
                plt.imshow(cumul_mat_norm,interpolation='nearest',vmin=0,vmax=thresh * cumul_mat_norm.max())
                fig.savefig(os.path.join(folder, str(level_pyr.level) + "_cumul_norm" + name_contig + '.pdf'))
                plt.close()
                ...
                plt.legend(['contact enrichment', 'ground truth'])
                fig.savefig(os.path.join(folder, str(level_pyr.level) + "_sum_profile_cumul_norm" + name_contig + '.pdf'))
                plt.close()
                fig = plt.figure(figsize=(siz_fig,siz_fig))
                ...
                plt.legend(['contact enrichment', 'ground truth'])
                fig.savefig(os.path.join(folder, str(level_pyr.level) + "_max_profile_cumul_norm" + name_contig + '.pdf'))
                plt.close()
    ...
    def manual_pre_detection(self):
        ...
        self.detection = dict()
        self.list_id_chrom = ultimate_pyr.dict_contigs.keys()
        ...
        for chrom_id in self.list_id_chrom:
            ...
            self.detection[chrom_id]["name"] = ultimate_pyr.dict_contigs[chrom_id]['name']
            for id_level in self.level.keys():
    ...
    def pre_detection(self, delta_bp):
        ...
        for chrom_id in self.list_id_chrom:
            ...
            for id_level in self.level.keys():
                ...
                self.detection[chrom_id][id_level]["bootstrap_norm"] = []
                self.detection[chrom_id][id_level]["bootstrap_raw"] = []
            self.detection[chrom_id]["pre_detect_pos"] = ultimate_pyr.all_localization[chrom_id]["kb"][chrom_id][0]
    ...
    def pre_detection_level(self, id_level, delta):
        ...
        for contig_index in level_pyr.dict_contigs:
            if not(contig_index == 17):
                print "analysing contig :", level_pyr.dict_contigs[contig_index]["name"]
                ...
                out_mean_pos_kb, out_position_kb, score, data_fit, out_mean_pos_pxl, out_position_pxl, data_fit_pxl = \
                    self.estimate_location_bloc_corr(bloc_corr, delta, tick_kb, sub_tick_kb, end_tick_kb)
                level_pyr.collect_bootstrap[contig_index]["kb"].append(out_mean_pos_kb)
                level_pyr.collect_bootstrap[contig_index]["pxl"].append(out_mean_pos_pxl)
                ...
                level_pyr.all_data[contig_index]["data_fit"]["kb"][contig_index] = data_fit
                level_pyr.all_data[contig_index]["data_fit"]["pxl"] = dict()
                level_pyr.all_data[contig_index]["data_fit"]["pxl"][contig_index] = data_fit_pxl
                level_pyr.all_data[contig_index]["score"][contig_index] = score


    def estimate_location_bloc_corr(self, bloc_corr, delta, tick_kb, sub_tick_kb, end_tick):
        score = []
        corrected_bloc = np.copy(bloc_corr)
        for i in xrange(0, corrected_bloc.shape[0]-1):
            corrected_bloc[i,i] = (corrected_bloc[i, i - 1] + corrected_bloc[i, i + 1]) / 2
        zoom_bloc_corr = np.copy(corrected_bloc)
        zoom_bloc_corr = zoom_bloc_corr ** 3
        for i in range(0, 2):
            score.append(0)
        for i in xrange(2, zoom_bloc_corr.shape[0]-2):
            is_self = True
        ...
        sub_pixel_pos_kb, data_fit_kb, sub_pixel_pos_pxl, data_fit_pxl = self.sub_pixel_pos(score, tick_kb, sub_tick_kb,
            end_tick)
        ...
        return sub_pixel_pos_kb, out_position_kb, score, data_fit_kb, sub_pixel_pos_pxl,out_position_pxl, data_fit_pxl

    def sub_pixel_pos(self, score, tickX, sub_tickX, end_tick):
        para_fit_kb, data_fit_kb = optim.gaussian_fit_kb(score, tickX, sub_tickX, end_tick)
    ...
    def la_cruz_azul_roja(self, im, x, y, delta, is_self):
        ...
        bloc_corr = im[x-delta_x_down:x+delta_x_up, y-delta_y_down:y+delta_y_up]
        ...
        id_cruz_roja[0].extend([i for i in xrange(x + delta_3, x, -1)])
        id_cruz_roja[1].extend([j for j in xrange(y - delta_3, y, +1)])
        delta_4 = min(delta_x_down, delta_y_up)
        id_cruz_roja[0].extend([i for i in xrange(x - 1, x - delta_4 - 1, -1)])
        id_cruz_roja[1].extend([j for j in xrange(y + 1, y + delta_4 + 1, +1)])

        if not(is_self):
            delta_1 = min(delta_x_down, delta_y_down)
            id_cruz_roja = [[i for i in xrange(x - delta_1, x)], [j for j in xrange(y - delta_1, y)]]
            delta_2 = min(delta_x_up, delta_y_up)
            id_cruz_roja[0].extend([i for i in xrange(x + 1, x + delta_2 + 1)])
            id_cruz_roja[1].extend([i for i in xrange(y + 1, y + delta_2 + 1)])

        if len(id_cruz_roja[0]) > 0 and id_cruz_roja[1] > 1:
            cruz_roja = im[id_cruz_roja]
            id_azul_y = range(y - azul_delta_y_down, y )
            id_azul_y.extend(range(y + 1, y + azul_delta_y_up + 1))
            id_azul_x = range(x - azul_delta_x_down, x )
            id_azul_x.extend(range(x + 1, x + azul_delta_x_up + 1))
            ...
            if is_self:
                score = (score_rojo / score_azul ) * std_azul * std_rojo * nuance_x[x] * bloc_corr.mean()

            else:
                score = val_al_lado[1, 1] * (score_rojo / score_azul) * std_azul * std_rojo * bloc_corr.mean()
        else:
            score_rojo = 0
            score_azul = 0
            score = 0
    ...
    def bootstrap_norm_data(self, delta_bp, size_filter_bp, N, thresh, fact_sub_sampling):
        """
        ...
        for id_level in keys_level:
            ...
            for i in xrange(0, N):
                ...
                self.refine_localization_4_boot(id_level, delta_bp, size_filter_bp, thresh)
                print "##########################"
            if not(level_pyr.is_max_level):
                cuda_gen.free_gpu()
                del cuda_gen
            for chrom_id_i in self.list_id_chrom:
                ...
                np.array(self.detection[chrom_id_i][id_level]["bootstrap_norm"])

    def bootstrap_raw_data(self, delta_bp, size_filter_bp, N_samples, thresh, fact_sub_sampling):
        """
...
########################################################################################################################
            for chrom_id_i in self.list_id_chrom:
                ...
                pre_pos_pxl_i = self.bp_coord_2_pxl_coord(pre_pos_bp_i, chrom_id_i, id_level)
                ...
                for chrom_id_j in self.list_id_chrom:
                    if chrom_id_i != chrom_id_j:
                        ...
                        pre_pos_pxl_j = self.bp_coord_2_pxl_coord(pre_pos_bp_j, chrom_id_j, id_level)
                ...
                subTick = sub_tick_kb[np.all([sub_tick_kb >= tick[0], sub_tick_kb <= tick[-1]], axis=0)]
                ...
                for i in xrange(0, N_samples):
                    ...
                    tmp = tmp - optim.gaussian_filter(tmp,delta, mode="constant", cval=0.0)
                    tmp[tmp < 0] = 0
                    tmp_thresh = np.copy(tmp)
#                    thresh = 0.2
                    tmp_thresh[tmp_thresh<tmp_thresh.max()*thresh] = tmp_thresh.max()*thresh
                    ########################################################################
                    tmp_thresh = optim.gaussian_filter(tmp_thresh,1,mode="constant",cval=0.0)
                    ########################################################################
                    data_boot = tmp_thresh.max(axis=1)
                    ########################################################################
#                    data_boot = tmp_thresh.sum(axis=1)
                    ########################################################################
                    coeff_x, data_fit = optim.gaussian_fit_kb(data_boot, tick, subTick, end_tick)
                    pos_kb = coeff_x[2]

                    self.detection[chrom_id_i][id_level]["bootstrap_raw"].append(pos_kb)
                self.detection[chrom_id_i][id_level]["cumul_mat_raw"] = cumul_mat
                self.detection[chrom_id_i][id_level]["bootstrap_raw"] = np.array(self.detection[chrom_id_i][id_level]["bootstrap_raw"])
...
#                    coord_ij = level_pyr.inter_coord[chrom_id_i][chrom_id_j]
                    ...
                    pre_pos_bp_j = self.detection[chrom_id_j]["pre_detect_pos"]
                    pre_pos_pxl_j = self.bp_coord_2_pxl_coord(pre_pos_bp_j, chrom_id_j, id_level)
                    cumul_mat = cumul_mat + data_i[:,coord_intra_j][pre_pos_pxl_i - delta: pre_pos_pxl_i + delta + 1,
                                  pre_pos_pxl_j - delta: pre_pos_pxl_j + delta + 1]
#                    cumul_mat = cumul_mat + image[coord_ij][pre_pos_pxl_i - delta: pre_pos_pxl_i + delta + 1,
#                                  pre_pos_pxl_j - delta: pre_pos_pxl_j + delta + 1]
...
#            cumul_mat = cumul_mat / base_contact
            cumul_mat = optim.gaussian_filter(cumul_mat, size_filter, mode="constant", cval=0.0)
            ...
            subTick = sub_tick_kb[np.all([sub_tick_kb >= tick[0], sub_tick_kb <= tick[-1]], axis=0)]
            cumul_mat = cumul_mat - optim.gaussian_filter(cumul_mat,delta, mode="constant", cval=0.0)
            cumul_mat[cumul_mat < 0] = 0
            cumul_mat =optim.gaussian_filter(cumul_mat,size_filter, mode="constant", cval=0.0)
            cumul_mat_thresh = np.copy(cumul_mat)
#            thresh = 0.2
            cumul_mat_thresh[cumul_mat_thresh<cumul_mat_thresh.max()*thresh] = cumul_mat_thresh.max()*thresh
            #####################################################################################
            cumul_mat_thresh = optim.gaussian_filter(cumul_mat_thresh,1,mode="constant", cval=0.0)
            #####################################################################################
            data_to_fit = cumul_mat_thresh.max(axis=1)
            #####################################################################################
#            data_to_fit = cumul_mat_thresh.sum(axis=1)
            #####################################################################################
            coeff_x, data_fit = optim.gaussian_fit_kb(data_to_fit, tick, subTick, end_tick)
            pos_kb = coeff_x[2]
    ...
    def plot_results(self, is_cerevisiae, delta_bp, fact_sub_sampling):
        ...
        for chrom in self.detection.keys():
...
#            f= plt.figure()
            ...
            for id_level in list_level:
                ...
                pos_centro_pxl = self.bp_coord_2_pxl_coord(pos_centro_kb,chrom,id_level)
                ...
                if lev_pyr.is_max_level:
                    ...
                    axImCorr.imshow(imCorr, interpolation="nearest", origin="lower", extent=extentCorr)
                    ...
                    axImCorr = plt.subplot(gs[id_lev, 0])
                    list_level[-1]
                    tick_score_kb = self.level[list_level[-1]].dict_contigs[chrom]["tick_kb"]
                    score = self.level[list_level[-1]].all_data[chrom]["score"][chrom]
                    score = np.array(score)
                    axImCorr.bar(tick_score_kb, score,linewidth=2,facecolor='r', edgecolor='r')
#                    axImCorr.bar(tick_score_kb, score, facecolor='r', edgecolor='r', linewidth=0, alpha=1)
                ...
                axCumSub_raw.imshow(imCumu_raw,interpolation='nearest',origin='lower', extent=extentCumu_raw, aspect='equal')
                ...
                axCumSub_norm.imshow(imCumu_norm,interpolation='nearest',origin='lower', extent=extentCumu_norm, aspect='equal')
                axCumSub_norm.set_xlim([0, imCumu_norm.shape[0]])
                axCumSub_norm.set_ylim([0, imCumu_norm.shape[1]])
                axCumSub_norm.set_title('Cumulated normalized inter matrix', fontsize=10)

                collect_pos_raw = self.detection[chrom][id_level]["bootstrap_raw"]
                axBootStrap_raw.hist( collect_pos_raw, facecolor='b', edgecolor='b', linewidth=0, alpha=0.5)
                ...
                axBootStrap_norm.hist( collect_pos_norm, facecolor='b', edgecolor='b', linewidth=0, alpha=0.5)
                ...
                if is_cerevisiae:
                    ...
                    axBootStrap_raw.axvspan(ground_truth[0], ground_truth[1], edgecolor='r', facecolor='r', alpha=1)
                    axBootStrap_raw.set_xlim([ground_truth[0] - deviation, ground_truth[1] + deviation])
                    axBootStrap_raw.text(0.8, 0.90, "mean error(bp) = " + str(np.around(error_loc_raw, 3)), fontsize=5,
                                     horizontalalignment='center',
                                     verticalalignment='center', transform=axBootStrap_raw.transAxes)
                    axBootStrap_raw.legend(["ground truth","bootstrap estimates"], loc='upper left', prop={'size':5})
                    axBootStrap_raw.set_title("Bootstrap distribution (raw data) " + "res = "+str(mean_size_pxl)+" bp",
                        fontsize=9)

                    error_loc_norm = np.min( np.abs(ground_truth - mean_position_norm))
                    axBootStrap_norm.axvspan(ground_truth[0], ground_truth[1], edgecolor='r', facecolor='r', alpha=1)
                    axBootStrap_norm.set_xlim([ground_truth[0] - deviation, ground_truth[1] + deviation])
                    axBootStrap_norm.text(0.8, 0.90, "mean error(bp) = " + str(np.around(error_loc_norm, 3)), fontsize=5,
                                     horizontalalignment='center',
                                     verticalalignment='center', transform= axBootStrap_norm.transAxes)
                    axBootStrap_norm.legend(["ground truth","bootstrap estimates"], loc='upper left', prop={'size':5})
                    axBootStrap_norm.set_title("Bootstrap distribution (norm data) " + "res = "+str(mean_size_pxl)+" bp",
                        fontsize=9)


                else:
                    axBootStrap_raw.axvspan(mean_position_raw - 200, mean_position_raw+200, edgecolor='r', facecolor='r', alpha=1)
                    axBootStrap_raw.legend(["mean position", "bootstrap estimates"], loc='upper left', prop={'size':5})
                    axBootStrap_raw.text(0.8, 0.90, "mean position(bp) = " +str(np.around(mean_position_raw, 3)), fontsize=5,
                                     horizontalalignment='center',
                                     verticalalignment='center', transform= axBootStrap_raw.transAxes)
                    axBootStrap_raw.set_xlim([mean_position_raw - delta_bp, mean_position_raw + delta_bp])
                    axBootStrap_raw.set_title("Bootstrap distribution (raw data)" + "res = "+str(mean_size_pxl)+" bp",
                        fontsize=9)

                    axBootStrap_norm.axvspan(mean_position_norm - 200, mean_position_norm+200, edgecolor='r', facecolor='r', alpha=1)
                    axBootStrap_norm.legend(["mean position", "bootstrap estimates"], loc='upper left', prop={'size':5})
                    axBootStrap_norm.text(0.8, 0.90, "mean position(bp) = " +str(np.around(mean_position_norm, 3)), fontsize=5,
                                     horizontalalignment='center',
                                     verticalalignment='center', transform= axBootStrap_norm.transAxes)
                    axBootStrap_norm.set_xlim([mean_position_norm - delta_bp, mean_position_norm + delta_bp])
                    axBootStrap_norm.set_title("Bootstrap distribution" + "res = "+str(mean_size_pxl)+" bp",
                        fontsize=9)



                axBootStrap_raw.text(0.8, 0.95, "precision(bp) = " +str(np.around(precision_raw, 3)), fontsize=5,
                                 horizontalalignment='center',
                                 verticalalignment='center', transform=axBootStrap_raw.transAxes)
                axBootStrap_norm.text(0.8, 0.95, "precision(bp) = " +str(np.around(precision_norm, 3)), fontsize=5,
                                 horizontalalignment='center',
                                 verticalalignment='center', transform=axBootStrap_norm.transAxes)
                id_lev -= 1
                aspectratio = 1.0
                ...
                ratio_boot_norm = float(axBootStrap_norm.get_xlim()[1]-axBootStrap_norm.get_xlim()[0])/float(axBootStrap_norm.get_ylim()[1]-axBootStrap_norm.get_ylim()[0])
                ...
                ratio_boot_raw = float(axBootStrap_raw.get_xlim()[1]-axBootStrap_norm.get_xlim()[0])/float(axBootStrap_raw.get_ylim()[1]-axBootStrap_raw.get_ylim()[0])
                ...
                ratio_cum_raw = float(axCumSub_raw.get_xlim()[1]-axCumSub_raw.get_xlim()[0])/float(axCumSub_raw.get_ylim()[1]-axCumSub_raw.get_ylim()[0])
                axCumSub_raw.set_aspect(ratio_cum_raw * aspectratio)
                plt.setp(axCumSub_raw.get_xticklabels(), visible=False)


                ratio_cum_norm=float(axCumSub_norm.get_xlim()[1]-axCumSub_norm.get_xlim()[0])/float(axCumSub_norm.get_ylim()[1]-axCumSub_norm.get_ylim()[0])
                ...
                if lev_pyr.is_max_level:
                    ratio_im_corr=float(axImCorr.get_xlim()[1]-axImCorr.get_xlim()[0])/float(axImCorr.get_ylim()[1]-axImCorr.get_ylim()[0])
            ...
            plt.draw()
            f.savefig(os.path.join(folder_sub_sampling, 'localization_' + name_chrom +'_' + str(fact_sub_sampling) + '.pdf'))
            # plt.close(f)
        val_mean_raw = []
        val_mean_norm = []
        for id_level in list_level:
            ...
            for i in xrange(1, len(list_name) + 1):
                ...
                if is_cerevisiae:
                    data_raw.append(t - (self.ground_truth[i][0] + self.ground_truth[i][0]) /2)
                else:
            ...
            if is_cerevisiae:
                ax1.set_title('Distribution of bootstrap estimates (raw data) VS groundtruth centromeric position')
            else:
                ax1.set_title('Distribution of bootstrap estimates (raw data) VS mean estimated centromeric position')
            f_raw.savefig(os.path.join(folder_sub_sampling, 'boxplot_raw_'+str(id_level) + '_' + str(fact_sub_sampling)
                                                            + '.pdf'))
            # plt.close(f_raw)

            file_id_level_bootstrap_raw = os.path.join(folder_sub_sampling, "bootstrap_positions_raw_"+str(id_level)) +\
                                          '_' + str(fact_sub_sampling) +".txt"
            ...
            if np.__version__ == '1.6.1':
                ...
                np.savetxt(file_id_level_bootstrap_raw, matbootstrap_raw.T, fmt='%.18e', delimiter='\t ')
            else:
                np.savetxt(file_id_level_bootstrap_raw, matbootstrap_raw.T, fmt='%.18e', delimiter='\t ', newline='\n', header=head, footer='', comments='')
            ...
            for i in xrange(1, len(list_name) + 1):
                ...
                if is_cerevisiae:
                    data_norm.append(t - (self.ground_truth[i][0] + self.ground_truth[i][0]) /2)
                else:
            ...
            plt.setp(xtickNames, rotation=45, fontsize=8)
            if is_cerevisiae:
                ax1.set_title('Distribution of bootstrap estimates (norm. data) VS groundtruth centromeric position')
            else:
                ax1.set_title('Distribution of bootstrap estimates (norm. data) VS mean estimated centromeric position')
            f_norm.savefig(os.path.join(folder_sub_sampling, 'boxplot_norm_'+str(id_level) + '_' + str(fact_sub_sampling)
                                                             +'.pdf'))
            # plt.close(f_norm)

            file_id_level_bootstrap_norm = os.path.join(folder_sub_sampling, "bootstrap_positions_norm_"+str(id_level))+\
                                           '_' + str(fact_sub_sampling) + ".txt"
            ...
            if np.__version__ == '1.6.1':
                ...
                np.savetxt(file_id_level_bootstrap_norm, matbootstrap_norm.T, fmt='%.18e', delimiter='\t ')
            else:
                np.savetxt(file_id_level_bootstrap_norm, matbootstrap_norm.T, fmt='%.18e', delimiter='\t ', newline='\n', header=head, footer='', comments='')
...
if __name__ == '__main__':
    ...
    pyramid_hic = pyr.build_and_filter(base_folder, size_pyramid, factor, min_bin_per_contig, size_chunk, max_level)
    ...
    if not (os.path.exists(output_folder)):
        os.mkdir(output_folder)
    output_folder = os.path.join(data_set_root, 'results', data_set[selected], 'centro_detect')
    if not (os.path.exists(output_folder)):
        os.mkdir(output_folder)
    ####################################################################################################################
    start = 2
