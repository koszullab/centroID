...
class GPUProcess(multiprocessing.Process):
...
def test_kernel(input_array_gpu):
    ...
    func(output_array_gpu, input_array_gpu, block=(512, 1, 1))
    cuda.memcpy_dtoh(output_array, output_array_gpu)
    return output_array


# cuda.init() #COMMENT HERE
some_array = numpy.ones((1, 512), dtype=numpy.float32)
# num = cuda.Device.count() #COMMENT HERE
num = 2

gpu_process_list = []
for i in range(num):
    gpu_process = GPUProcess(i, some_array)
    gpu_process.start()
    gpu_process_list.append(gpu_process)
