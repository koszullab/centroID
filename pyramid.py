...
def build_and_filter(
    base_folder, size_pyramid, factor, min_bin_per_contig, size_chunk, default_level
):
    ...
    if not (
        ...
        thresh = remove_problematic_fragments(
            contig_info,
            fragments_list,
    ...
    for level in range(0, size_pyramid):
        ...
        if not (status):
            ...
            level_to_fill = pyramid_handle.create_dataset(
                str(level), (nfrags, nfrags), "i"
            )
            fill_pyramid_level(
                level_to_fill, new_abs_fragments_contacts_file, size_chunk, nfrags
            )
...
def fill_pyramid_level(hdf5_data, abs_contacts_file, size_chunk, nfrags):
    ...
    chunk_points = range(0, nfrags, size_chunk)
    ...
    if fact_sub_sample <= 1:
        ...
        if not (abs_fragments_contacts == "SIMU"):
            ...
            while 1:
                ...
                w_sub_sample = (
                    spec_new_frags[new_abs_id_frag_a]["accu_frag"]
                    * spec_new_frags[new_abs_id_frag_b]["accu_frag"]
                )
                handle_new_abs_fragments_contacts.write(
                    "%s\t%s\t%s\t%s\t%s\n"
                    % (
                        str(new_abs_id_frag_a),
                        str(new_abs_id_frag_b),
                        w_size,
                        w_gc,
                        str(w_sub_sample),
                    )
...
def remove_problematic_fragments(
    ...
    list_init_contig = list(init_contigs.keys())
...
class pyramid:
    def __init__(self, pyramid_folder, n_levels, default_level):
        ...
        for i in range(0, n_levels):
            ...
            frag_dictionary, contig_dictionary = self.build_frag_dictionnary(
            ...
            if find_super_index:
                ...
                for contig_id in list(self.spec_level[str(i)]["contigs_dict"].keys()):
                    try:
                        t = int(contig_id)
                    except ValueError:
    ...
    def zoom_in_pixel(self, curr_pixel):
        ...
        if level > 0:
            str_level = str(level)
            low_sub_low = self.spec_level[str_level]["fragments_dict"][low_frag][
                "sub_low_index"
            ]
            low_sub_high = self.spec_level[str_level]["fragments_dict"][low_frag][
                "sub_high_index"
            ]
            high_sub_low = self.spec_level[str_level]["fragments_dict"][high_frag][
                "sub_low_index"
            ]
            high_sub_high = self.spec_level[str_level]["fragments_dict"][high_frag][
                "sub_high_index"
            ]
    ...
    def zoom_out_pixel(self, curr_pixel):
        ...
        if level < self.n_level - 1:
            ...
            high_super = self.spec_level[str_level]["fragments_dict"][high_frag][
                "sub_index"
            ]
