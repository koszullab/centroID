...
class contig:
    ...
    def initiate(cls, init_contig, id_frag_start, id_frag_end, orientation):
        obj = cls()
        obj.name = obj.init_contig_name(
            init_contig, id_frag_start, id_frag_end, orientation
        )
        obj.length_kb = 0
        obj.n_frags = 0
        obj.kb_metric = [0]
        ...
        if reverse:
            for i in range(len(self.FragmentS) - 1, -1, -1):
                ...
                out = (
                    out
                    + f.init_name
                    + f.orientation.translate(string.maketrans("wc", "cw"))
                )
        else:
    ...
    def update_repeats(self, old_instance_2_new_instance):
        ...
        for ele in self.new_compo:
            if ele["dup"]:
                ...
                new_contig_id = old_instance_2_new_instance[old_contig_id][
                    "new_instance"
                ]
    ...
    def return_extremity(self, extremity, orientation):
        ...
        sub_parts = list(self.old_2_new.keys())
        first_coords = []
        for ele in sub_parts:
            tmp = ele.split(":")
            first_coords.append(int(tmp[0]))
        new_index = sorted(
            list(range(len(first_coords))), key=lambda k: first_coords[k]
        )
    ...
    def sub_content(self, pos_a, pos_b):
        ...
        for ele in range(0, n_compo):
            ...
            if orientation == "w":
                ...
                index_frag = list(range(1, n_frags_local + 1))
            else:
                #                index_frag = range(end,start-1,-1)
                index_frag = list(range(n_frags_local, 0, -1))
    ...
    def split_name(self, pos_cut):
        ...
        for ele in range(0, n):
            ...
            n_frags = (end - start) + 1
            if orientation == "w":
                index_frag = list(range(1, n_frags + 1))
                start_write = start
            else:
                index_frag = list(range(n_frags, 0, -1))
                start_write = end
            id = 0
            out_cond = True
            for i in index_frag:
                ...
                if (abs_pos >= pos_a) and (abs_pos <= pos_b):
                    ...
                    if orientation == "w":
                        name = (
                            ">"
                            + string.join(
                                [init_contig, str(start_write), str(i), orientation],
                                ":",
                            )
                            + ">"
                        )
                        ...
                        name = (
                            ">"
                            + string.join(
                                [init_contig, str(i), str(start_write), orientation],
                                ":",
                            )
                            + ">"
                        )
    ...
    def reverse_content(self,):
        ...
        for ele in init_contigs_compo:
            ...
            ele["orientation"] = ele["orientation"].translate(
                string.maketrans("wc", "cw")
            )
    ...
    def split(cls, contig_a, pos_cut):
        ...
        frag1, compo1, new_compo1, frag2, compo2, new_compo2 = contig_a.split_content(
            pos_cut
        )
