...
def fitgaussian(data,tickX):
    ...
    errorfunction = lambda p: np.ravel(gaussian(*p)(*grid) -
                                 data)
...
def gaussian_fit_restricted(data_raw,):
    ...
    data = data - B_e_x
    data[data < 0] = 0
    bin_centres_x = np.array(range(0, len(data_raw)))
...
def gmm_fit_kb(data_raw, init_sigma):
    ...
    opt_fun = lambda x, *p: p[0] * np.exp(-(x - p[1]) ** 2 / (2. * p[2] ** 2)) + p[3] * np.exp(-(x - p[4]) ** 2 / (2. * p[5] ** 2))
    try:
        coeff_x, var_matrix_x = curve_fit(opt_fun, bin_centres_x, data, p0=p0_x)
    except RuntimeError:
        print "optimal parameters not found"
...
def gaussian_fit_pxl(data_raw, tickX, sub_tickX):
    ...
    if tickX[-1] - tickX[0] > 70000:
        data[index_extrem] = 0
    bin_centres_x = range(0, len(tickX))
...
#                    tick_kb.append(start)
        ...
        for id_cont in xrange(1, len(self.dict_contigs)+1):
            ...
            if not (id_cont == 17):
               ...
               intra = self.trans_free_im_init[np.ix_(coord_cont[id_cont], coord_cont[id_cont])]
        ...
        for id_cont in self.dict_contigs:
            if not (id_cont == 17):
                ...
                if self.is_max_level:
                    mat_tmp = self.trans_free_im_init[np.ix_(coord_cont[id_cont], coord_cont[id_cont])]
                    mat_tmp[:, :] = self.mean_value_trans
                    mat_tmp = np.random.poisson(mat_tmp)
                    self.trans_free_im_init[np.ix_(coord_cont[id_cont], coord_cont[id_cont])] = mat_tmp
                else:
                    self.trans_free_im_init[np.ix_(coord_cont[id_cont], coord_cont[id_cont])] = self.mean_value_trans
            ...
            if not (id_cont_X == 17):
                ...
                for id_cont_Y in self.dict_contigs:
                    if not (id_cont_Y == 17):
                        ...
                        self.all_data[id_cont_X]["sub_matrices"][id_cont_Y]["raw"] = sub_mat_raw
                        self.all_data[id_cont_X]["sub_matrices"][id_cont_Y]["corr"] = sub_mat_corr
...
#                full_raw_matrix = np.copy(self.im_trans_free_curr[coord_intra, :])
                intra_raw_matrix = np.copy(self.im_curr[np.ix_(coord_intra, coord_intra)])

                inter_raw_matrix = np.copy(full_raw_matrix)
                inter_raw_matrix[:, coord_intra] = 0

                full_corr_matrix = np.copy(self.corr_im[coord_intra, :])
                intra_corr_matrix = np.copy(self.corr_im[np.ix_(coord_intra, coord_intra)])
...
class analysis():
    ...
    def single_bootstrap(self, id_level, delta_bp, size_filter_bp):
        ...
        level_pyr = self.level[id_level]
        level_pyr.update_data_scn()
        output = self.refine_localization_4_boot_multi_proc(id_level, delta_bp, size_filter_bp)
        return output
    def analyse_bootstrap(self,id_level, ground_truth):
        ...
        for id_chr in level_pyr.dict_contigs.keys():
            pos_chr = np.int32(id_chr)
            ind_x = int((pos_chr - 1) / sizX)
            ind_y = (pos_chr - 1) - ind_x * sizX
            curr_gs = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=gs0[ind_y, ind_x])
            self.plot_results(level_pyr, id_chr, fig, curr_gs, ground_truth)
        plt.draw()
        fig.savefig(os.path.join(level_pyr.output_folder, 'boot_strap_analysis_' + str(id_level) + '.pdf'))
        plt.close()
    ...
    def old_plot_results(self, level_pyr, id_chr, fig, curr_gs, ground_truth):
        ...
        axBStrapCorr.hist(pos_bootstrap, facecolor='b', edgecolor='b', linewidth=0, alpha=0.5)
        if ground_truth:
            ground_truth = self.ground_truth[id_chr]
            axBStrapCorr.axvspan(ground_truth[0], ground_truth[1], edgecolor='r', facecolor='r', alpha=1)
        ...
        axImCorr.imshow(im_corr, interpolation='nearest',extent=[min_bound, max_bound, min_bound, max_bound],
                        origin='lower')
        ...
        for chrom_id_i in self.list_id_chrom:
            ...
            for chrom_id_j in self.list_id_chrom:
                if chrom_id_i != chrom_id_j:
                    ...
                    tmp = raw_sub[pre_pos_pxl_i - delta: pre_pos_pxl_i + delta + 1,
...
########################################################################################################################
    ...
    def post_detection(self, id_level, delta, N_samples,size_filter, is_cerevisiae):
        ...
        level_pyr = self.level[id_level]
        for id_chrom_i in level_pyr.dict_contigs.keys():
            ...
            pos_centro_i = round(level_pyr.all_localization[id_chrom_i]["pxl"][id_chrom_i][1].mean())
            axImCorr = axarr[0, 0]
            axImCCorr = axarr[0, 1]
            ...
            axImCCorr.imshow(np.corrcoef(imCorr), interpolation="nearest", origin="lower")
            ...
            for id_chrom_j in level_pyr.dict_contigs.keys():
                if id_chrom_i != id_chrom_j:
                    raw_sub = data_i[id_chrom_j]["raw"]
                    pos_centro_j = round(level_pyr.all_localization[id_chrom_j]["pxl"][id_chrom_j][1].mean())
                    cumul_mat = cumul_mat + raw_sub[pos_centro_i - delta: pos_centro_i + delta + 1,
            ...
            axBootStrap.hist(collect_pos,facecolor='b', edgecolor='b', linewidth=0, alpha=0.5)
            ...
            if is_cerevisiae:
                ...
                axBootStrap.axvspan(ground_truth[0], ground_truth[1], edgecolor='r', facecolor='r', alpha=1)
                axBootStrap.set_xlim([ground_truth[0] - 10000, ground_truth[1] + 10000])
                axBootStrap.text(0.8, 0.90, "mean error(bp) = " + str(np.around(error_loc, 3)), fontsize=7, horizontalalignment='center',
                                     verticalalignment='center', transform = axBootStrap.transAxes)
                axBootStrap.legend(["ground truth","bootstrap estimates"], loc='upper left', prop={'size':5})
                axBootStrap.set_title("Bootstrap distribution and ground truth")
            else:
                axBootStrap.axvspan(mean_position - 200, mean_position+200, edgecolor='r', facecolor='r', alpha=1)
                axBootStrap.set_title("Bootstrap distribution")
                axBootStrap.legend(["mean position", "bootstrap estimates"], loc='upper left', prop={'size':6})
                axBootStrap.text(0.8, 0.90, "mean position(bp) = " +str(np.around(mean_position, 3)), fontsize=7, horizontalalignment='center',
                             verticalalignment='center', transform = axBootStrap.transAxes)
                axBootStrap.set_xlim([mean_position - 10000, mean_position + 10000])
            axBootStrap.text(0.8, 0.95, "precision(bp) = " +str(np.around(precision, 3)), fontsize=7, horizontalalignment='center',
                             verticalalignment='center', transform = axBootStrap.transAxes)
            ...
            plt.draw()
            f.savefig(os.path.join(level_pyr.output_folder, 'localization_' + name_contig + '.pdf'))
    ...
    def refine_localization(self, delta_bp, size_filter_bp, N_samples):
        ...
        for id_level in keys_level:
            ...
            for chrom_id_i in self.list_id_chrom:
                ...
                pre_pos_pxl_i = self.bp_coord_2_pxl_coord(pre_pos_bp_i, chrom_id_i, id_level)
                sub_inter_mat = full_inter_mat[pre_pos_pxl_i - delta: pre_pos_pxl_i + delta + 1,:]
...
#                    data_fit_pxl = self.estimate_location_bloc_corr(tmp_corr, mon_delta, tick, sub_tick)
                    ...
                    self.detection[chrom_id_i][id_level]["bootstrap"].append(pos_kb)
                self.detection[chrom_id_i][id_level]["cumul_mat"] = gaussian_filter(tmp,1)
                self.detection[chrom_id_i][id_level]["bootstrap"] = np.array(self.detection[chrom_id_i][id_level]["bootstrap"])
    ...
    def plot_results(self, is_cerevisiae, delta_bp):
        ...
        for chrom in self.detection.keys():
            ...
            for id_level in list_level:
                ...
                self.detection[chrom][id_level]["bootstrap"] = np.array(self.detection[chrom][id_level]["bootstrap"],dtype=np.float32)
                ...
                if is_cerevisiae:
                    ...
                    error_loc = np.min( np.abs(ground_truth - mean_position))
                    axBootStrap.axvspan(ground_truth[0], ground_truth[1], edgecolor='r', facecolor='r', alpha=1)
                    axBootStrap.set_xlim([ground_truth[0] - deviation, ground_truth[1] + deviation])
#                    axBootStrap.set_xlim([ground_truth[0] - delta_bp, ground_truth[1] + delta_bp])
                    axBootStrap.text(0.8, 0.90, "mean error(bp) = " + str(np.around(error_loc, 3)), fontsize=5,
                                     horizontalalignment='center',
                                     verticalalignment='center', transform= axBootStrap.transAxes)
                    axBootStrap.legend(["ground truth","bootstrap estimates"], loc='upper left', prop={'size':5})
                    axBootStrap.set_title("Bootstrap distribution and ground truth",fontsize=10)
                else:
                    axBootStrap.axvspan(mean_position - 200, mean_position+200, edgecolor='r', facecolor='r', alpha=1)
                    axBootStrap.legend(["mean position", "bootstrap estimates"], loc='upper left', prop={'size':5})
                    axBootStrap.text(0.8, 0.90, "mean position(bp) = " +str(np.around(mean_position, 3)), fontsize=5,
                                     horizontalalignment='center',
                                     verticalalignment='center', transform= axBootStrap.transAxes)
                    axBootStrap.set_xlim([mean_position - delta_bp, mean_position + delta_bp])
                    axBootStrap.set_title("Bootstrap distribution", fontsize=10)
                axBootStrap.text(0.8, 0.95, "precision(bp) = " +str(np.around(precision, 3)), fontsize=5,
                                 horizontalalignment='center',
                                 verticalalignment='center', transform=axBootStrap.transAxes)
                id_lev += 1

            for a in xrange(0, len(list_level) - 1 ):
                for b in xrange(1, 3):
            ...
            plt.draw()
            f.savefig(os.path.join(self.all_res_folder, 'localization_' + name_chrom + '.pdf'))
            plt.close()
        val_mean = []
        for id_level in list_level:
            data = []
            ...
            for i in xrange(1, len(list_name) + 1):
                ...
                if is_cerevisiae:
                    data.append(t - (self.ground_truth[i][0] + self.ground_truth[i][0]) /2)
                else:
            ...
            if is_cerevisiae:
                ax1.set_title('Distribution of bootstrap estimates VS groundtruth centromeric position')
            else:
                ax1.set_title('Distribution of bootstrap estimates VS mean estimated centromeric position')
            f.savefig(os.path.join(self.all_res_folder, 'boxplot_'+str(id_level) + '.pdf'))
            plt.close()
            fviolin = plt.figure(figsize=(10, 10))
            ax_violin = fviolin.add_subplot(111)
            violin_plot(ax_violin, data, range(1, len(list_name) + 1), bp=True)
            fviolin.savefig(os.path.join(self.all_res_folder, 'violin_plot_'+str(id_level) + '.pdf'))
            plt.close()
            file_id_level_bootstrap = os.path.join(self.all_res_folder, "bootstrap_positions_"+str(id_level))+".txt"
            ...
            if np.__version__ == '1.6.1':
                ...
                np.savetxt(file_id_level_bootstrap, matbootstrap.T, fmt='%.18e', delimiter='\t ')
            else:
                np.savetxt(file_id_level_bootstrap, matbootstrap.T, fmt='%.18e', delimiter='\t ', newline='\n', header=head, footer='', comments='')
        ...
        for contig_index in level_pyr.dict_contigs:
            if not(contig_index == 17):
                ...
                out_mean_pos_kb, out_position_kb, score, data_fit, out_mean_pos_pxl, out_position_pxl, data_fit_pxl = \
                    self.estimate_location_bloc_corr(bloc_corr, delta, tick_kb, sub_tick_kb)
...
#    A.bootstrap(id_level=1,delta_bp=delta_bp,size_filter_bp=size_filter_bp,N=N_samples)
    A.bootstrap(id_level=2,delta_bp=delta_bp,size_filter_bp=size_filter_bp,N=N_samples)
    A.bootstrap(id_level=3,delta_bp=delta_bp,size_filter_bp=size_filter_bp,N=N_samples)
